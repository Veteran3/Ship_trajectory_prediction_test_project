...
class Model(nn.Module):
    ...
    def forward(self, x_enc, y_truth_abs, mask_x, mask_y, x_static=None): # x_static 设为可选
        """
        前向传播 (已升级为 "V3 - 最佳实践" 策略)
        
        Args:
            x_enc (torch.Tensor): [B, T_in, N, D_in] - 绝对历史轨迹
            y_truth_abs (torch.Tensor): [B, T_out, N, 2] - 真实绝对坐标
        ...
        # 1. 准备 掩码 和 静态嵌入 (已修正, x_static 为可选)
        ...
        if x_static is not None:
             ...
             # 如果没有 x_static, 确保 node_encoder 存在
             assert hasattr(self, 'node_encoder'), "模型中既没有 static_encoder 也没有 node_encoder"
        ...
        if x_static is not None:
            enc_in = enc_in + static_embed_expanded
        else:
        ...
        if self.training:
            ...
            # [关键修改]
            # 我们需要 "右移" 的绝对坐标作为 Decoder 输入
            last_known_pos = x_enc_permuted[:, :, -1:, :2]
            prev_future_pos = y_truth_abs.permute(0, 2, 1, 3)[:, :, :-1, :]
            
            # y_input_abs 是 Decoder 的输入 (Teacher Forcing)
            y_input_abs = torch.cat([last_known_pos, prev_future_pos], dim=2) # [B, N, T_out, 2]
            ...
            # 准备当前步的掩码
            temporal_mask_self = temporal_padding_mask_dec
            attn_mask_self_step = attn_mask_self
            ...
            # 输出仍然是 "预测的增量"
            output = self.output_proj(dec_out) # [B, N, T_out, 2]
            
        else:
            # ========================
            # 推理: 自回归 (已修改)
            # ========================
            output = self.autoregressive_decode(
                memory, 
                entity_padding_mask, 
                temporal_padding_mask_enc, 
                attn_mask_self, 
                x_enc_permuted, # 历史
                x_static # 静态特征
            )
        
        # 4. Reshape 输出 & 最终掩码 (不变)
