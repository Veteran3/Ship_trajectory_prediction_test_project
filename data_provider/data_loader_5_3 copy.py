...
class LaneFeatureCache:
    ...
    def __init__(self, root_path, flag, data_shape, lane_config_hash):
        self.cache_dir = os.path.join(root_path, 'lane_features_cache_v5_3')
...
def get_lane_start_direction(lane_name, lanes):
    """
    获取指定航道的起始方向向量 (dx, dy)
    """
    ...
    if 'center' not in lane_data:
        return 0.0, 0.0
        
    line = lane_data['center']
...
def next_lanes_to_dir_features(next_lanes, lanes):
    """
    将下一航道列表转换为方向特征 (cos1, sin1, cos2, sin2)
    支持最多 2 个分叉航道。不足 2 个补 0。
    """
    feats = np.zeros(4, dtype=np.float32)
    
    if isinstance(next_lanes, str):
        next_lanes = [next_lanes]
    elif next_lanes is None:
        next_lanes = []
        
    # 处理列表中的每个航道（最多取前2个）
    count = 0
    for lane_name in next_lanes:
        if count >= 2: break
        if not lane_name: continue
            
        dx, dy = get_lane_start_direction(lane_name, lanes)
        feats[count*2] = dx
        feats[count*2+1] = dy
        count += 1
...
def _compute_lane_features_chunk(args):
    ...
    # 修改：lane_dir_feats 扩充为 4 维
    lane_dir_feats = np.zeros((B_chunk, T, N, 4), dtype=np.float32)
    # next_lanes_feats 保持 8 维
        ...
        for t in range(T):
            for n in range(N):
                ...
                try:
                    ...
                    # 3.方向特征 (这里原本是计算当前航道方向，现在修改为计算 下一航道 的方向)
                    # 获取 seq
                    ...
                    # 计算下一航道的方向特征 (4维)
                    dir_feats_vec = next_lanes_to_dir_features(next_lanes, lanes)
                    lane_dir_feats[local_idx, t, n, :] = dir_feats_vec
                    
                    # 4.Next lanes One-Hot 特征 (保持不变)
...
class LaneLookupCache:
    ...
    def __init__(self, root_path, flag, data_shape, lane_table_path):
        self.cache_dir = os.path.join(root_path, 'lane_lookup_cache_v5_3')
        os.makedirs(self.cache_dir, exist_ok=True)
...
class ShipTrajectoryDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.lanes is not None and self.lane_lookup_cache is not None:
            ...
            # 修改：4维
            lane_dir_feats = np.zeros((T_in, N, 4), dtype=np.float32)
    ...
    def _compute_lane_features_runtime(self, index, seq_x, seq_x_mask):
        ...
        # 修改：4 维
        lane_dir_feats = np.zeros((T, N, 4), dtype=np.float32)
        ...
        for t in range(T):
            for n in range(N):
                ...
                try:
                    ...
                    dir_feats_vec = next_lanes_to_dir_features(next_lanes, self.lanes)
                    lane_dir_feats[t, n, :] = dir_feats_vec
