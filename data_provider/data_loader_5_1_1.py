...
"""
v5.1 深度优化版 (Precision Optimized)
核心策略：
...
class LaneLookupCache:
    ...
    def __init__(self, root_path, flag, data_shape, lane_table_path):
        # 使用 v5_1_opt 后缀以示区别
        self.cache_dir = os.path.join(root_path, 'lane_lookup_cache_v5_1_opt_2')
...
def densify_coordinates(coords, step_meters=100):
    """
    [升级版] 使用三次样条插值 (Cubic Spline) 对航道进行平滑加密
    这能消除线性插值带来的"折线拐点"，生成平滑的曲线。
    """
    if len(coords) < 2:
        return coords
    ...
    # 2. 计算累积距离 (作为插值的自变量 t)
    # 粗略估算经纬度距离 (如果你有更精确的墨卡托投影最好，没有的话欧氏距离凑合也行)
    dx = np.diff(x)
    dy = np.diff(y)
    dist = np.sqrt(dx**2 + dy**2)
    cum_dist = np.insert(np.cumsum(dist), 0, 0) # [0, d1, d1+d2, ...]
    
    total_len = cum_dist[-1]
    
    # 3. 如果只有两个点，样条插值没意义，退化为线性
    if len(coords) <= 2:
        # 计算插值点数 (degree ~ 0.001 deg per 100m)
        step_deg = step_meters / 111000.0 
        num_steps = max(2, int(total_len / step_deg))
        new_x = np.linspace(x[0], x[-1], num_steps)
        new_y = np.linspace(y[0], y[-1], num_steps)
        
    else:
        # 4. 构建样条函数 (参数化插值: x(t), y(t))
        # bc_type='natural' 保证端点平滑
        cs_x = CubicSpline(cum_dist, x, bc_type='natural')
        cs_y = CubicSpline(cum_dist, y, bc_type='natural')
        
        # 5. 生成密集的 t 值
        step_deg = step_meters / 111000.0 
        # 稍微多采一点点，保证覆盖全长
        new_cum_dist = np.arange(0, total_len, step_deg) 
        if new_cum_dist[-1] < total_len:
            new_cum_dist = np.append(new_cum_dist, total_len)
            
        # 6. 计算新的坐标
        new_x = cs_x(new_cum_dist)
        new_y = cs_y(new_cum_dist)
        ...
        for t in range(T):
            ...
            for n in range(N):
                ...
                try:
                    ...
                    # 2. 获取序号 (v5.1 逻辑需要用到 seq 来找 next_lanes)
...
class ShipTrajectoryDataset(Dataset):
    ...
    def __precompute_lane_lookup__(self):
        ...
        cache = LaneLookupCache(self.root_path, self.flag, (B, T, N, D), self.lane_table_path)
        
        # === 检查缓存 ===
        if not self.force_recompute and cache.is_valid():
            print(f"[{self.flag}] Found valid cache. Loading directly (No progress bar needed)...")
            self.lane_lookup_cache = cache.load()
            return
        
        # === 开始计算 ===
        print(f"\n[{self.flag}] Start Computing Lane Lookup (Force Recompute={self.force_recompute})...")
        print(f"          Total Samples: {B:,}")
        print(f"          Using Cores: {min(cpu_count(), 8)}")
        
        lon_phys = self.data_x[:, :, :, 0] * self.std[0] + self.mean[0]
        lat_phys = self.data_x[:, :, :, 1] * self.std[1] + self.mean[1]
        
        num_processes = min(cpu_count(), 8)
        # 保持较小的 chunk_size 以便进度条平滑更新
        chunk_size = 20  
        sample_chunks = [list(range(i, min(i + chunk_size, B))) for i in range(0, B, chunk_size)]
        
        chunk_args = []
        for chunk in sample_chunks:
            chunk_args.append((chunk, lon_phys[chunk], lat_phys[chunk], self.mask_x[chunk], self.lanes, self.df_lanes))
        ...
        # === TQDM 进度条 ===
        # ncols=100: 固定宽度，防止换行
        # mininterval=0.5: 至少0.5秒刷新一次，防止刷新太快闪烁
        with Pool(processes=num_processes) as pool:
            result_iter = pool.imap(_lookup_lanes_chunk, chunk_args)
            for chunk_result in tqdm(result_iter, total=len(sample_chunks), desc="Computing Lanes", unit="chunk", ncols=100, mininterval=0.5):
                all_results.extend(chunk_result)
                    
        print(f"  ✓ Lookup completed in {time.time() - start_time:.1f}s")
        ...
        for t in range(T):
            for n in range(N):
                ...
                try:
                    ...
                    # 注意：这是与 v5.2+ 最大的区别，v5.2 用的是 next_lanes 的方向
...
if __name__ == '__main__':
    ...
    args = Args()
    print("Testing Optimized v5.1 Loader...")
